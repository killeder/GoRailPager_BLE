...
#define CC1101_SCK_CLOCK    RCC_AHBPeriph_GPIOA 	//SPI的SCK引脚PA5
#define CC1101_SCK_PORT     GPIOA
#define CC1101_SCK_PIN      GPIO_Pin_5
#define CC1101_SCK_SOURCE   GPIO_PinSource5
#define CC1101_SCK_AF       GPIO_AF_0

#define CC1101_MISO_CLOCK	RCC_AHBPeriph_GPIOA	//SPI的MISO引脚PA6
#define CC1101_MISO_PORT	GPIOA
#define CC1101_MISO_PIN		GPIO_Pin_6
#define CC1101_MISO_SOURCE  GPIO_PinSource6
#define CC1101_MISO_AF      GPIO_AF_0

#define CC1101_MOSI_CLOCK	RCC_AHBPeriph_GPIOA	//SPI的MOSI引脚PA7
#define CC1101_MOSI_PORT	GPIOA
#define CC1101_MOSI_PIN		GPIO_Pin_7
#define CC1101_MOSI_SOURCE  GPIO_PinSource7
#define CC1101_MOSI_AF      GPIO_AF_0

#define CC1101_CS_CLOCK 	RCC_AHBPeriph_GPIOA	//SPI的片选引脚PA4
...
static void HwSPI_Init(void)
{
	...
	//1.初始化SPIx使用到的四个引脚
	RCC_AHBPeriphClockCmd(CC1101_SCK_CLOCK|CC1101_MISO_CLOCK|
	...
	GPIO_InitStructure.GPIO_PuPd  = GPIO_PuPd_NOPULL;//取消上下拉
	GPIO_Init(CC1101_SCK_PORT,&GPIO_InitStructure);//设置SCK引脚
	GPIO_PinAFConfig(CC1101_SCK_PORT, CC1101_SCK_SOURCE, CC1101_SCK_AF);

	GPIO_InitStructure.GPIO_Pin = CC1101_MISO_PIN;
	GPIO_Init(CC1101_MISO_PORT,&GPIO_InitStructure);//设置MISO引脚
	GPIO_PinAFConfig(CC1101_MISO_PORT, CC1101_MISO_SOURCE, CC1101_MISO_AF);

	GPIO_InitStructure.GPIO_Pin = CC1101_MOSI_PIN;
	GPIO_Init(CC1101_MOSI_PORT,&GPIO_InitStructure);//设置MOSI引脚
	GPIO_PinAFConfig(CC1101_MOSI_PORT, CC1101_MOSI_SOURCE, CC1101_MOSI_AF);
	...
	SPI_RxFIFOThresholdConfig(CC1101_SPIx, SPI_RxFIFOThreshold_QF);
...
static void SPISendByte(uint8_t data)
{
	...
	SPI_SendData8(CC1101_SPIx, data);	//发送一字节数据
	while(SPI_I2S_GetFlagStatus(CC1101_SPIx, SPI_I2S_FLAG_RXNE) == RESET)
	{
	}	//等待接收缓冲区空
	SPI_ReceiveData8(CC1101_SPIx);	//收一字节数据（状态字）并舍弃
	CRIS_EXIT();
}
/*-----------------------------------------------------------------------
*@brief		SPI接收一字节数据
...
static uint8_t SPIReadByte(void)
{
	...
	SPI_SendData8(CC1101_SPIx, 0xFF);	//发送一字节Dummy数据
	while(SPI_I2S_GetFlagStatus(CC1101_SPIx, SPI_I2S_FLAG_RXNE) == RESET)
	{
	}	//等待接收缓冲区空
	retval = SPI_ReceiveData8(CC1101_SPIx);	//收一字节数据
	CRIS_EXIT();
...
static void GDO2_ExtIRQ_Init(void)
{
	...
	//1.初始化GDO2连接到的引脚
	RCC_AHBPeriphClockCmd(CC1101_IRQ_CLOCK,ENABLE);//打开外部中断引脚的时钟
	...
	SYSCFG_EXTILineConfig(CC1101_IRQ_PORTSOURCE,CC1101_IRQ_PINSOURCE);//选择EXTI的信号源	
	EXTI_InitStructure.EXTI_Line = CC1101_IRQ_LINE; //选择相应中断线
	EXTI_InitStructure.EXTI_Mode = EXTI_Mode_Interrupt;	//选择为中断模式
	EXTI_InitStructure.EXTI_Trigger = EXTI_Trigger_Falling;	//下降沿触发
	//GDO2常态为Low，同步码匹配时被CC1101拉为High，完整数据包接收完成后落回Low
	//因此要设置与GDO2连接的引脚为下降沿触发，确保在包结束后触发中断
	EXTI_InitStructure.EXTI_LineCmd = ENABLE;	//使能中断线
	EXTI_Init(&EXTI_InitStructure);

	NVIC_InitStructure.NVIC_IRQChannel = CC1101_IRQ_IRQn;
	NVIC_InitStructure.NVIC_IRQChannelPriority = 0;//设为最高优先级
